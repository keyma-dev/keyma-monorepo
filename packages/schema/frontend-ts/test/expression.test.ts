...
const FIXTURES = path.join(__dirname, "..", "..", "..", "frontend-ts", "test", "fixtures");
const SNAPSHOTS = path.join(__dirname, "..", "..", "..", "frontend-ts", "test", "snapshots");
...
describe("getters — newly-supported portable expressions", () => {
    ...
    it("lowers an object literal", () => {
        const expr = lowered(`
            import { Schema, Computed } from "@keyma/schema/dsl";
            import type { Json } from "@keyma/schema/dsl";
            @Schema() class Foo {
...
describe("expression lowering — null literal", () => {
    it("lowers null literal to { kind: 'literal', value: null }", () => {
        const result = cv({
            "schema.ts": `
                import { Schema, Computed } from "@keyma/schema/dsl";
                import type { Nullable } from "@keyma/schema/dsl";
                @Schema() class Foo {
