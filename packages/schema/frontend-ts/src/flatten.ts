...
function flattenSchema(
    ...
    // Merge composite indexes: concatenate and deduplicate (both ride in the
    // schema-domain `extensions['schema']` slice now).
    const mergedIndexes = mergeIndexes(schemaIndexes(flatParent), schemaIndexes(schema));
    ...
    // Rebuild the schema-domain slice with the merged indexes, preserving the child's own
    // ephemeral/edge. `...rest` shallow-copied `extensions` by reference, so route through
    // `setSchemaExtSlice` (which writes a fresh `extensions` object) to avoid mutating the
    // pre-flatten child node's extension.
    const sExt: SchemaExtData = { ...(schemaExt(schema) ?? {}) };
    if (mergedIndexes.length > 0) sExt.indexes = mergedIndexes;
    else delete sExt.indexes;
    setSchemaExtSlice(out, sExt);
    return out;
}
