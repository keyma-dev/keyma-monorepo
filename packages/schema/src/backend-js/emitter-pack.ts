...
/**
 * The schema-domain JS emitter pack: supplies the per-class `<Class>.metadata` builder and the
 * `ClassMetadata` runtime type surface. The validator/formatter factories are ORDINARY functions
 * emitted by the generic backend (no claimed-wrapper rendering): the synthesized
 * `validate()`/`format*()` methods reference them through the IR call graph (tree-shaken
