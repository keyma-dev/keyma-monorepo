...
// disk: the TypeScript standard library `.d.ts` set plus the `@keyma/dsl`,
// `@keyma/validators` and `@keyma/formatters` sources, laid out under a virtual
// `/node_modules/@keyma/*` tree so NodeNext module resolution resolves them in-memory.
...
const KEYMA_PACKAGES = ["@keyma/dsl", "@keyma/validators", "@keyma/formatters"] as const;
