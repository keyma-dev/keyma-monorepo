...
describe("KEYMA014 — unsupported computed getter expressions", () => {
    it("emits KEYMA014 for a call expression in a getter body", () => {
        const result = cv({
            "schema.ts": `
                ...
                @Schema() class Foo {
                    get name(): string { return compute(); }
                }
            `,
        });
        ...
        const result = cv({
            "schema.ts": `
                ...
                @Schema() class Foo {
                    ...
                    get name(): string { return this.helper(); }
                }
            `,
        });
        assert.ok(hasError(result, CODES.KEYMA014), `Expected KEYMA014. Got: ${JSON.stringify(result.diagnostics)}`);
    ...
    it("emits KEYMA014 for a new expression in a getter body", () => {
        const result = cv({
            "schema.ts": `
                import { Schema } from "@keyma/dsl";
                @Schema() class Foo {
                    get created(): Date { return new Date(); }
                }
            `,
        });
        assert.ok(hasError(result, CODES.KEYMA014), `Expected KEYMA014. Got: ${JSON.stringify(result.diagnostics)}`);
    ...
    it("emits KEYMA014 for an object literal in a getter body", () => {
        const result = cv({
            "schema.ts": `
                import { Schema } from "@keyma/dsl";
                @Schema() class Foo {
                    get meta(): object { return {}; }
                }
            `,
        });
        assert.ok(hasError(result, CODES.KEYMA014), `Expected KEYMA014. Got: ${JSON.stringify(result.diagnostics)}`);
    ...
    it("emits KEYMA014 for a typeof expression in a getter body", () => {
        const result = cv({
            "schema.ts": `
                import { Schema } from "@keyma/dsl";
                @Schema() class Foo {
                    declare n: number;
                    get typeLabel(): string { return typeof this.n; }
                }
            `,
        });
        assert.ok(hasError(result, CODES.KEYMA014), `Expected KEYMA014. Got: ${JSON.stringify(result.diagnostics)}`);
...
// ─── KEYMA015 — getter with matching setter ───────────────────────────────────

describe("KEYMA015 — computed getter must have no setter", () => {
    ...
    it("does not emit KEYMA015 for a getter without a setter", () => {
        const result = cv({
            "schema.ts": `
                import { Schema } from "@keyma/dsl";
                @Schema() class Foo {
    ...
    it("lowers null literal to { kind: 'literal', value: null }", () => {
        const result = cv({
            "schema.ts": `
                ...
                @Schema() class Foo {
                    get nothing(): Nullable<string> { return null; }
                }
            `,
        });
