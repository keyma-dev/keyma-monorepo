...
// considered. The server uses this to drop `id` on create and to validate only the supplied
// fields on a partial update (so absent fields never trip the required check).
template <class Pred>
std::pmr::vector<ValidationError> validate_if(const SchemaMeta& schema, const Value& value,
...
inline std::pmr::vector<ValidationError> validate(const SchemaMeta& schema, const Value& value, alloc_t a) {
