...
function f(name: string, type: IRType, extra: Partial<IRField> = {}): IRField {
    return { name, type, visibility: "public", readonly: false, required: true, validators: [], formatters: [], indexes: [], source: src("user.ts"), ...extra };
}
...
const Tag: IRSchema = {
    ...
    indexes: [{ fields: [{ name: "id", direction: 1 }], unique: true }], source: src("tag.ts"),
};
