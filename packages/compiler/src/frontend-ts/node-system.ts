...
// disk: the TypeScript standard library `.d.ts` set plus the `@keyma/core` and
// `@keyma/schema` sources (the latter provides `@keyma/schema/dsl`, `@keyma/schema/validators`
// and `@keyma/schema/formatters`), laid out under a virtual `/node_modules/@keyma/*` tree so
// NodeNext module resolution resolves them in-memory.
...
/** The Keyma packages a virtual schema may import, and which on-disk files to vendor in. */
const KEYMA_PACKAGES = ["@keyma/core", "@keyma/schema"] as const;

/**
 * Build a fully in-memory `ts.System` pre-loaded with the TS lib files and the Keyma
 * authoring packages, read from disk once. The result is the browser-capable virtual
 * filesystem; feed it to `compileVirtual(sources, { system })`.
 */
export function createKeymaNodeSystem(opts: NodeSystemOptions = {}): ts.System {
    ...
    for (const name of KEYMA_PACKAGES) {
