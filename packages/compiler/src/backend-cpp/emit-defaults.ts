...
/**
 * Build a free function applying a class's field defaults (literal AND expression)
 * to a record `keyma::Value`, referenced from the class metadata (server bundles
...
export function buildApplyDefaults(cls: IRClassDeclaration, includePrivate: boolean): { name: string; def: string } | null {
    ...
    const name = `apply_defaults_${factoryIdent(cls.sourceName)}`;
