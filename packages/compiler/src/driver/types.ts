...
export type KeymaUserConfig = {
    ...
    irOutFile?: string;
    /** Prefix prepended to every generated class/service `name` (the canonical
     *  identity used for references, registries, RPC and serialization).
     *  Namespaces the project so the same class names can coexist across
     ...
     *  emit `IRMember.tag`. JSON-only projects leave this off (the default). */
    binary?: boolean;
    ...
    /** Domain packages to load (each a package exporting a `keymaDomain` descriptor). The
     *  CLI resolves each named package's descriptor and wires it across the extension seams. Omit to
...
export type ResolvedConfig = {
    ...
    outDir: string;
    irOutFile?: string;
    /** Prefix for every class/service `name`. See {@link KeymaUserConfig.namePrefix}. */
    namePrefix: string;
    targets: KeymaTargetConfig[];
