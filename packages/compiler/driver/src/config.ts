...
export function resolveConfig(user: KeymaUserConfig): ResolvedConfig {
    ...
    return {
        ...
        // Preserve the distinction between "omitted" (auto-detect) and an explicit list
        // (including `[]` → core only): only carry `domains` through when the user set it.
        ...(user.domains !== undefined ? { domains: user.domains } : {}),
    };
}
